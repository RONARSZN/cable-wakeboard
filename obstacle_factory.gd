...
static func create_pipe_rail() -> Node3D:
	...
	var left_support = _create_support("LeftSupport", -3.5)
	var right_support = _create_support("RightSupport", 3.5)
	obstacle.add_child(left_support)
	obstacle.add_child(right_support)
	return obstacle
